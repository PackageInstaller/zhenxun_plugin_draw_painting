...
async def handle_wives_draw(bot: Bot, event: Event):
    ...
    game_names_in_library = {
        img.split("_")[0].lower() for img in all_images if "_" in img
    }  # 只考虑带有_的游戏名

    if game_name:
        game_name_parts = list(game_name)
        fuzzy_matches = await improved_partial_word_match(
            game_name_parts, game_names_in_library
        )
        if fuzzy_matches:
            ...
            closest_match, similarity = process.extractOne(
                game_name, game_names_in_library, scorer=fuzz.partial_ratio
            )
            ...
            if similarity < 70:
                try:
                    ...
                    await bot.send(
                        event, f"发送消息时发生错误：{str(e)}", reply_message=True
                    )
                await wives_draw.finish()
        ...
        selected_wives = (
            db_handler.get_selected_wives_or_husbands_by_game(closest_match, "Wife")
            if closest_match
            else db_handler.get_all_selected_wives_or_husbands("Wife")
        )
        selected_characters = {
            "_".join(name.split("_")[1:2]).lower() for name in selected_wives
        }
        game_images = [
            img for img in all_images if closest_match in img.split("_")[0].lower()
        ]
        ...
        image_list = [
            ...
            if "_" in img
            and "_".join(os.path.splitext(img)[0].split("_")[1:2]).lower()
            not in selected_characters
        ]
    else:
        selected_wives = (
            db_handler.get_selected_wives_or_husbands_by_game(game_name, "Wife")
            if game_name
            else db_handler.get_all_selected_wives_or_husbands("Wife")
        )
        selected_characters = {
    ...
    matching_images = [
        ...
        if os.path.splitext(img)[0].startswith(match_prefix)
        and (
            os.path.splitext(img)[0] == match_prefix
            or os.path.splitext(img)[0][len(match_prefix)] == "_"
        )
    ]
...
async def handle_husbands_draw(bot: Bot, event: Event):
    ...
    if game_name:
        ...
        selected_husbands = (
            db_handler.get_selected_wives_or_husbands_by_game(closest_match, "Husband")
            if closest_match
            else db_handler.get_all_selected_wives_or_husbands("Husband")
        )
        selected_characters = {
            "_".join(name.split("_")[1:2]).lower() for name in selected_husbands
        }
        game_images = [
            img for img in all_images if closest_match in img.split("_")[0].lower()
        ]
        ...
        selected_husbands = (
            db_handler.get_selected_wives_or_husbands_by_game(game_name, "Husband")
            if game_name
            else db_handler.get_all_selected_wives_or_husbands("Husband")
        )
        selected_characters = {
...
async def handle_wives_view(bot: Bot, event: Event):
    ...
    if not result:
        try:
            await bot.send(
                event, f"你还没有老婆呢，快发送 抽老婆 来抽取吧", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
    ...
    if not matching_images:
        try:
            await bot.send(
                event, "找不到你老婆的图片呢，请确认是否已被删除", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
        await wives_view.finish()
...
async def handle_husbands_view(bot: Bot, event: Event):
    ...
    if not result:
        try:
            await bot.send(
                event, f"你还没有老公呢，快发送 抽老公 来抽取吧", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
    ...
    if not matching_images:
        try:
            await bot.send(
                event, "找不到你老公的图片呢，请确认是否已被删除", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
        await husbands_view.finish()
...
async def handle_wives_rename(bot: Bot, event: Event, state: T_State):
    ...
    matching_images = [
        img
        for img in os.listdir(wives_images_folder)
        if os.path.splitext(img)[0] == stored_name
    ]

    if not matching_images:
        try:
            await bot.send(
                event, f"找不到你老婆的图片呢，请确认是否已被删除", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
        await wives_rename.finish()
...
async def handle_got_new_wives_name(bot: Bot, event: Event, state: T_State):
    ...
    if state.get("awaiting_name") and state.get("user_id") == str(event.get_user_id()):
        ...
        if not new_name:
            try:
                await bot.send(
                    event, f"名字不能为空，请输入有效的名字", reply_message=True
                )
            except Exception as e:
                await bot.send(
                    event, f"发送消息时发生错误：{str(e)}", reply_message=True
        ...
        for img in matching_images:
            ...
            if img_suffix == stored_image_suffix:
                ...
                db_handler.update_renamed_record(
                    state["user_id"], os.path.splitext(img)[0], new_name
                )
            else:
                new_base_name = (
...
async def handle_husbands_rename(bot: Bot, event: Event, state: T_State):
    ...
    if not matching_images:
        try:
            await bot.send(
                event, f"找不到你老公的图片呢，请确认是否已被删除", reply_message=True
            )
        except Exception as e:
            await bot.send(event, f"发送消息时发生错误：{str(e)}", reply_message=True)
        await husbands_rename.finish()
    ...
    if state.get("awaiting_name") and state.get("user_id") == str(event.get_user_id()):
        ...
        if new_game_name != original_game_name:
            try:
                await bot.send(
                    event,
                    f"新名字中的游戏名与原始游戏名不一致，请保持游戏名一致。原游戏名为 {original_game_name}",
                    reply_message=True,
                )
            except Exception as e:
                await bot.send(
...
async def handle_husbands_probability(bot: Bot, event: Event):
    ...
    try:
        ...
        all_images = [
            f for f in os.listdir(husbands_images_folder) if f.count("_") >= 1
        ]
        if not all_images:
            await bot.send(event, "没有找到任何有效的图片", reply_message=True)
            await wives_probability.finish()
...
async def handle_delete_husbands(bot: Bot, event: Event):
    ...
    if girl_confidence > 0.6:
        ...
        message_content = (
            f"来自{source_info}的用户{user_nickname}({user_id})"
    ...
    if deleted_images:
        ...
        try:
            ...
            db_handler.log_moved_record(
                user_id, "delete_husbands", deleted_images, "Husband"
            )
        except Exception as e:
...
async def handle_help_confirmation(bot: Bot, event: Event):
    ...
    try:
        ...
        message = MessageSegment.image(buf) + MessageSegment.text(
        ...
        if event.notice_type == "group_msg_emoji_like":
            ...
            if (
                ...
                if emoji_id == "38":  # 同意
                    ...
                    reply_msg = (
                        MessageSegment.reply(confirmation.original_message_id)
                        + "您已选择不同意，将无法使用相关功能。如需使用，请重新触发指令并同意霸王条款。"
                    )
                    await bot.send(event, reply_msg)
...
async def handle_emoji_vote(bot: Bot, event: NoticeEvent):
    ...
    if event.notice_type == "group_msg_emoji_like":
        ...
        if message_id in ongoing_votes:
            ...
            # 已经投过票，不允许再投
            if (
                user_id in vote_data["agree"]
                or user_id in vote_data["disagree"]
                or user_id in vote_data["abstain"]
            ):
                return
...
async def finalize_vote(
    bot: Bot, vote_data, message_id, event, early_termination=False, owner_vote=None
):
    """终止计时并统计投票结果"""
    try:
        ...
        if vote_result == "删除图片":
            ...
            for img in matching_images:
                img_path = (
                    os.path.join(husbands_images_folder, img)
                    if img in os.listdir(husbands_images_folder)
                    else os.path.join(wives_images_folder, img)
                )
                ...
                try:
                    ...
                    await bot.send(
                        event,
                        f"删除图片 {img} 时发生错误：{str(e)}",
                        reply_message=True,
                    )

            db_record = db_handler.get_card_name(user_id)
            if db_record and any(img_name in card for card in db_record):
                db_handler.delete_draw_record(
                    user_id, "Husband" if "Husband" in img_name.lower() else "Wife"
                )
...
async def handle_vote_delete(
    ...
    if state["reply_message_id"]:
        ...
        if state["reply_message_id"] in ongoing_votes or any(
            vote["image_name"] == image_name for vote in ongoing_votes.values()
        ):
            await bot.send(
                event, "该图片已经有正在进行的投票，无法重复发起。", reply_message=True
            )
    ...
    if image_owner_id is None:
        await bot.send(
            event, "无法获取图片的拥有者，无法发起投票。", reply_message=True
        )
        await vote_delete.finish()
